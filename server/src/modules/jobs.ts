...
const COLUMNS = 'id, title, company, raw_text, parsed, created_at, updated_at'
...
// PUT /api/jobs/:id → upsert. Body: { title, company, rawText, parsed }.
jobs.put('/:id', async (req, res) => {
  const { id } = req.params
  const { title = null, company = null, rawText = null, parsed = {} } = req.body ?? {}
  if (!title) return res.status(400).json({ error: 'title is required' })
  const { rows } = await pool.query(
    `INSERT INTO job_descriptions (id, title, company, raw_text, parsed, updated_at)
     ...
     ON CONFLICT (id) DO UPDATE SET
       ...
       updated_at = now()
     RETURNING ${COLUMNS}`,
    [id, title, company, rawText, JSON.stringify(parsed)],
  )
  res.json(rows[0])
})

...
// PUT /api/jobs/:id → upsert. Body adds optional application + prepPlan to the picks.
  ...
  const { rows } = await pool.query(
    `INSERT INTO job_descriptions
     ...
     ON CONFLICT (id) DO UPDATE SET
       ...
       coding_picks = EXCLUDED.coding_picks,
    ...
    [
      ...
      JSON.stringify(problemPicks),
      JSON.stringify(codingPicks),
      JSON.stringify(behavioralPicks),
      JSON.stringify(recruiterPicks),

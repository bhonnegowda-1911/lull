...
const DOT: Record<StageStatus, string> = {
  ...
  current: 'bg-terracotta-600 text-white',
  skipped: 'bg-stone-300 text-white',
  upcoming: 'bg-white text-stone-400 border border-stone-300',
}
...
export default function StageTracker({
  ...
  return (
    <div className="rounded-xl border border-stone-200/80 bg-[#fcfaf6] p-4 shadow-sm">
      <h3 className="text-sm font-semibold text-stone-700">{heading}</h3>
            ...
            <li key={stage.id} className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1 pb-1">
                <div className="flex items-baseline justify-between gap-2">
                  <span
                    className={`text-sm ${isCurrent ? 'font-semibold text-terracotta-800' : 'text-stone-700'}`}
                  >
                    {stage.label}
                  </span>
                  <span className="shrink-0 text-[11px] text-stone-400">~{stage.minutes}m</span>
                </div>
                {isCurrent && <p className="text-xs text-stone-500">{stage.goal}</p>}
              </div>
            </li>
          )
        })}
